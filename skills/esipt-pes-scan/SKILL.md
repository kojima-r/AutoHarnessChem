...
Relaxed Scan を実行する。専用 conda 環境 `pyscf` で動く。
...
- **error_type=missing_dependency（geometric）**: 拘束付き構造最適化には geomeTRIC が必要。
  `conda run -n pyscf pip install geometric` が必要で、agent 側では修復不能。
  代替として `calculate_tddft_spectrum` で個別構造の励起エネルギーを比較する方法を提案する。
...
- **error_type=timeout**: スキャン範囲を分割して 2 回に分ける（`start_dist`/`end_dist`）か
  `step_size` を粗くする。`timeout_sec` を上げてもよい。
